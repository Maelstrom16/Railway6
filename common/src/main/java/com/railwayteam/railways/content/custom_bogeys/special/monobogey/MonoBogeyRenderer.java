...
public class MonoBogeyRenderer extends StandardBogeyRenderer.Small {
    @Override
    public void render(CompoundTag bogeyData, float wheelAngle, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int light, int overlay, boolean inContraption) {
        ...
        CachedBuffers.partial(MONOBOGEY_FRAME, Blocks.AIR.defaultBlockState()) 
                .rotateZDegrees(specialUpsideDown ? 180 : 0)
                .translateY(specialUpsideDown ? -3 : 0)
                .light(light)
                .overlay(overlay)
                .renderInto(poseStack, buffer);

        SuperByteBuffer wheel = CachedBuffers.partial(MONOBOGEY_WHEEL, Blocks.AIR.defaultBlockState());
        for (boolean left : Iterate.trueAndFalse) {
            for (int front : Iterate.positiveAndNegative) {
