...
public class Medium606TrailingRenderer extends StandardBogeyRenderer.Small {
    ...
    public void render(CompoundTag bogeyData, float wheelAngle, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int light, int overlay, boolean inContraption) {
        ...
        for (int i : Iterate.zeroAndOne) {
            secondaryShaft.translate(-.5f, .31f, .5f + i * -2)
                    .center()
        ...
        CachedBuffers.partial(MEDIUM_6_0_6_TRAILING_FRAME, Blocks.AIR.defaultBlockState()) 
                .light(light)
                .overlay(overlay)
                .renderInto(poseStack, buffer);

        SuperByteBuffer wheel = CachedBuffers.partial(MEDIUM_SHARED_WHEELS, Blocks.AIR.defaultBlockState());
