...
public class WideScotchYokeBogeyRenderer extends StandardBogeyRenderer.Large {
    ...
    public void render(CompoundTag bogeyData, float wheelAngle, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int light, int overlay, boolean inContraption) {
        ...
        CachedBuffers.partial(WIDE_SCOTCH_FRAME, Blocks.AIR.defaultBlockState()) 
                .translate(0, 0, 0)
                .light(light)
                .overlay(overlay)
                .renderInto(poseStack, buffer);

        CachedBuffers.partial(WIDE_SCOTCH_PISTONS, Blocks.AIR.defaultBlockState()) 
                .translate(0, 1, 1 / 4f * Math.sin(AngleHelper.rad(wheelAngle)))
                .light(light)
                .overlay(overlay)
                .renderInto(poseStack, buffer);

        CachedBuffers.partial(WIDE_SCOTCH_WHEELS, Blocks.AIR.defaultBlockState()) 
        ...
        CachedBuffers.partial(WIDE_SCOTCH_PINS, Blocks.AIR.defaultBlockState()) 
                .translate(0, 1, 0)
                .rotateXDegrees(wheelAngle)
                .translate(0, 1 / 4f, 0)
                .rotateXDegrees(-wheelAngle)
