...
public class LargeCreateStyled080Renderer extends StandardBogeyRenderer.Large {
    ...
    public void render(CompoundTag bogeyData, float wheelAngle, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int light, int overlay, boolean inContraption) {
        ...
        for (int side : Iterate.positiveAndNegative) {
            secondaryShaft.translate(-.5f, .25f, -.5f + side * 3.617)
                    .center()
        ...
        for (int side = -2; side < 3; side++) {
            ...
            middleShaft.translate(-.5f, .25f, -.5f + side * -1.6)
                    .center()
        ...
        SuperByteBuffer innerWheel = CachedBuffers.partial(LC_STYLE_SEMI_BLIND_WHEELS, Blocks.AIR.defaultBlockState());
