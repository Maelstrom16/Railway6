...
public class CRBlockPartials {
    ...
    static {
        for (DyeColor color : DyeColor.values()) {
            TOOLBOX_BODIES.put(color, createBlock(CreateLang.asId(color.name()) + "_toolbox"));
            CONDUCTOR_WHISTLE_FLAGS.put(color, block("conductor_whistle/flag_"+CreateLang.asId(color.name())));
        }
    }
